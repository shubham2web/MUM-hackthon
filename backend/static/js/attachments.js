            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (ai_analysis) {
                        ...
                        Messages.addAIMessage(`✅ "${file.name}" processed successfully, but no analysis was generated.`);
                    }
                } else {
                    Messages.addAIMessage(`❌ OCR Error for "${file.name}": ${result.error || 'Unknown error'}`);
                }

            } catch (error) {
                console.error('❌ OCR Error:', error);
                Messages.hideLoading();
                Messages.addAIMessage(`❌ Failed to process image: ${error.message}`);
            }
        },
