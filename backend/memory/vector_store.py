...
class VectorStore:
    ...
    def __init__(
        ...
        enable_reranking: bool = False,  # STEP 8: Cross-encoder reranking (experimental, LTR conflicts detected)
        ...
        reranker_fusion_weight: float = 0.7,  # STEP 8: Weight for vector scores in reranking fusion (70% vector, 30% cross-encoder)
    ):
        """
        ...
        Args:
            ...
            reranker_fusion_weight: Weight for vector scores in reranking (0.7 = 70% vector, 30% cross-encoder)
        """
        ...
        self.reranker_fusion_weight = reranker_fusion_weight
        ...
        # Initialize HGB Soft Bias Re-ranker (Step 4: DISABLED - conflicts with cross-encoder)
        # STEP 9b: Disabled to eliminate interference with Step 8 cross-encoder reranking
        self.ltr_reranker = None
        self.enable_ltr_reranking = False  # Hardcoded False - old LTR conflicts with new reranker
        
        if False:  # Permanently disabled - kept for reference
            try:
        ...
        # STEP 8: Initialize lightweight cross-encoder reranker
        self.cross_encoder_reranker = None
        if self.enable_reranking and LightweightReranker is not None:
            try:
                self.cross_encoder_reranker = LightweightReranker(
                    model_name="cross-encoder/ms-marco-MiniLM-L-6-v2",
                    fusion_weight=self.reranker_fusion_weight,
                    enabled=True
                )
                self.logger.info(f"✅ Cross-encoder reranker initialized (fusion_weight={self.reranker_fusion_weight})")
            except Exception as e:
                self.logger.warning(f"⚠️ Cross-encoder reranker not available: {e}")
                self.cross_encoder_reranker = None
    ...
    def search(
        ...
        try:
            if self.backend == "chromadb":
                ...
                # STEP 9b: LTR reranking DISABLED to eliminate cross-encoder interference
                # Old HGB-based LTR conflicts with Step 8 cross-encoder reranking
                # Kept code for reference but never executed (enable_ltr_reranking = False)
                if False and self.enable_ltr_reranking and self.ltr_reranker and blended_results:
                ...
                # STEP 8: Apply cross-encoder reranking for precision boost
                if self.cross_encoder_reranker and blended_results:
                    try:
                        ...
                        reranked_dicts = self.cross_encoder_reranker.rerank_results(
                            query=query,
                        ...
                        blended_results = [
                            RetrievalResult(
                                id=r['id'],
                                text=r['text'],
                                score=r['score'],  # Updated hybrid score from reranker
                                metadata=r['metadata'],
                                rank=r.get('rank', i+1),
                        ...
                        self.logger.info(f"✅ Cross-encoder reranking applied: {len(blended_results)} results reranked")
                    except Exception as e:
